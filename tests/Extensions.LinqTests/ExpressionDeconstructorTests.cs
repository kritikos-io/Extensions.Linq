...
public class ExpressionDeconstructorTests
{
  ...
  public void DeconstructBinary()
  {
    ...
    Assert.Throws<ArgumentNullException>(() => expr!.Deconstruct(out _, out _));

    expr = x => x + initial;

    var restructured = expr switch
    {
      LambdaExpression(var param, BinaryExpression(ExpressionType.Add, var left,
        ConstantExpression(ExpressionType.Constant, _, initial))) => Expression.Lambda(
          Expression.Add(left, Expression.Constant(addInstead)), param.ToArray()),
